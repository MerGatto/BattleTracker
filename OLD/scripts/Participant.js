class Participant
{
    ...
    {
        this.edge = true;
        $(this.row).addClass('edge');
        $(this.row).find('.btnEdge').hide();
    }
    ...
    syncValuesFromRow()
    {
        this.baseIni = convertToInt($(this.row).find('.baseIni')[0].value);
        if (this.baseIni != 0)
        {
    ...
    syncValuesToRow()
    {
        $(this.row).find('.baseIni')[0].value = this.baseIni;
        if (this.baseIni != 0)
        {
            $(this.row).removeClass('awaitingINI')
        }
    ...
    softReset(revive = false)
    {
        ...
        this.fullDefense = false;
        $(this.row).removeClass('edge');
        $(this.row).find('.btnEdge').show();
